...
def convert_psd_to_html(zip_path):
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        existing_dirs = glob.glob(os.path.join(public_dir, "output_*"))
        ...
        outputrepo = f"output_{timestamp}" 
        output_dir_base = os.path.join("public", outputrepo)
        ...
        def image_clip_path_generate(image_path, child_layer):
            try:
                ...
                # cv2.imwrite(f"public/{output_dir}/{file_name_t}/images/{child_layer.name}_edges.png", edges)
                cv2.imwrite(os.path.join(output_dir_base, file_name_t, "images", f"{child_layer.name}_edges.png"), edges)
                contours, _ = cv2.findContours(edges, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
                ...
                if not contours:
                    ...
                    for i, contour in enumerate(contours, 1):
                        ...
                        if shape == "Rectangle":
                            file_path_r = [
                                image_path,
                                # f"public/{output_dir}/{file_name_t}/images/{child_layer.name}_edges.png"
                                os.path.join(output_dir_base, file_name_t, "images", f"{child_layer.name}_edges.png")
                            ]
                    ...
                    # cv2.imwrite(f"public/{output_dir}/{file_name_t}/images/{child_layer.name}_contours.png", image)
                    cv2.imwrite(os.path.join(output_dir_base, file_name_t, "images", f"{child_layer.name}_contours.png"), image)
                    
                    print(f"Saved annotated image: {child_layer.name}_contours.png")
                    file_path_r = [
                        image_path,
                        # f"public/{output_dir}/{file_name_t}/images/{child_layer.name}_edges.png",
                        # f"public/{output_dir}/{file_name_t}/images/{child_layer.name}_contours.png"   
                        os.path.join(output_dir_base, file_name_t, "images", f"{child_layer.name}_edges.png"),
                        os.path.join(output_dir_base, file_name_t, "images", f"{child_layer.name}_contours.png")
                    ] 
                    for path_r in file_path_r:
                        os.remove(path_r)
                    return clip_path
        ...
        for psd_file in psd_files:  
            ...
            output_dir = os.path.join(output_dir_base, file_name_t)
            ...
            def process_layer(layer, html_content, css_content, content_html_app):
                ...
                if not layer.is_group() and layer.composite():
                    ...
                    if "logo" in layer.name:
                        ...
                        image_path = os.path.join(output_dir, "images", f"{sanitized_name}.png")
                        if cnt == 0:
                            try:
                    ...
                    for name in shape_names:
                        if name in layer.name:
                            ...
                            if layer.kind == "shape" or hasattr(layer, 'smart_object'):
                                ...
                                # image_path = f"public/{output_dir}/{file_name_t}/images/{sanitized_name}.png"
                                image_path = os.path.join(output_dir, "images", f"{sanitized_name}.png")
                        ...
                        if "hero" in layer.name and "hero 2" not in layer.name:
                            ...
                            for idx, child_layer in enumerate(reversed(layer)):
                                        ...
                                        if child_layer.origination:
                                            for shape in child_layer.origination:
                                                if 'RoundedRectangle' in str(shape):
                                                    ...
                                                    if wrapp_image:
                                                        # image_path = f"public/{output_dir}/{file_name_t}/images/{child_layer.name}.png"
                                                        image_path = os.path.join(output_dir, "images", f"{child_layer.name}.png")
                                ...
                                if pp.is_visible():
                                    ...
                                    # image_path = f"public/{output_dir}/{file_name_t}/images/{file_update_name}.jpg"
                                    image_path = os.path.join(output_dir, "images", f"{file_update_name}.jpg")
                                    try:
                        ...
                        if "hero 2" in layer.name:
                            ...
                            for idx, child_layer in enumerate(reversed(layer), start=1):
                                ...
                                if pp.is_visible():
                                    ...
                                    file_update_name = re.sub(r'\s+', '-', child_layer.name.strip())
                                    # image_path = f"public/{output_dir}/{file_name_t}/images/{file_update_name}{idxImageTwo}.jpg"
                                    image_path = os.path.join(output_dir, "images", f"{file_update_name}{idxImageTwo}.jpg")
